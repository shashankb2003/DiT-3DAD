...
from models.autoencoder_dit import DiTAutoEncoder
...
parser.add_argument('--dit_window_block_indexes', type=str, default='', help='Comma-separated list of block indexes to use window attention (e.g., "0,4,8")')
...
if args.resume is not None:
    ...
    if args.model_type == 'dit':
        model = DiTAutoEncoder(ckpt['args']).to(args.device)
    else:
        model = getattr(sys.modules[__name__], args.model)(ckpt['args']).to(args.device)
    model.load_state_dict(ckpt['state_dict'])
else:
    if args.model_type == 'dit':
        logger.info('Using DiT-based model')
        model = DiTAutoEncoder(args).to(args.device)
    else:
